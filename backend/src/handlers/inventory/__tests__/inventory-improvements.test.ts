import type { APIGatewayProxyEvent } from 'aws-lambda';

const mockSend = jest.fn();
jest.mock('@aws-sdk/client-dynamodb', () => ({ DynamoDBClient: jest.fn() }));
jest.mock('@aws-sdk/lib-dynamodb', () => ({
  DynamoDBDocumentClient: { from: () => ({ send: mockSend }) },
  QueryCommand: jest.fn((input) => ({ ...input, kind: 'query' })),
  GetCommand: jest.fn((input) => ({ ...input, kind: 'get' })),
  PutCommand: jest.fn((input) => ({ ...input, kind: 'put' })),
  UpdateCommand: jest.fn((input) => ({ ...input, kind: 'update' })),
  DeleteCommand: jest.fn((input) => ({ ...input, kind: 'delete' })),
}));
import { handler } from '../inventory';

function event(overrides: Partial<APIGatewayProxyEvent>): APIGatewayProxyEvent {
  return {
    httpMethod: 'GET',
    path: '/inventory/search',
    resource: '/inventory/search',
    pathParameters: null,
    queryStringParameters: null,
    body: null,
    requestContext: { authorizer: { claims: { sub: 'test-user' } }, requestId: 'test' },
    ...overrides,
  } as APIGatewayProxyEvent;
}

beforeEach(() => mockSend.mockReset());

it.each(['barcode', 'name'])(
  'finds the latest matching lot across all pages for %s',
  async (field) => {
    const old = {
      itemId: 'old',
      name: 'Rice',
      barcode: '5901234123457',
      category: 'Grains',
      unit: 'g',
      createdAt: '2026-01-01',
      updatedAt: '2026-09-04',
    };
    const latest = {
      ...old,
      itemId: 'latest',
      createdAt: '2026-09-01',
      updatedAt: '2026-09-01',
      expirationDate: '2028-01-01',
      pictureUrl: 'https://example.com/rice.jpg',
      locationDetails: 'Shelf 2A',
    };
    const cursor = { PK: 'USER#test-user', SK: 'ITEM#old' };
    mockSend
      .mockResolvedValueOnce({ Items: [old], LastEvaluatedKey: cursor })
      .mockResolvedValueOnce({ Items: [latest] });
    const result = await handler(
      event({ queryStringParameters: { field, query: field === 'name' ? 'ric' : '590' } }),
    );
    expect(result.statusCode).toBe(200);
    expect(JSON.parse(result.body).items).toEqual([latest]);
    expect(mockSend.mock.calls[1][0].ExclusiveStartKey).toEqual(cursor);
  },
);

it('persists a copied photo and shelf on a new lot while retaining the group threshold', async () => {
  const group = {
    groupId: 'rice',
    unit: 'g',
    totalQuantity: 500,
    threshold: 1,
    thresholdUnit: 'kg',
    isLowStock: true,
  };
  mockSend
    .mockResolvedValueOnce({ Item: group })
    .mockResolvedValueOnce({})
    .mockResolvedValueOnce({ Item: group })
    .mockResolvedValueOnce({ Attributes: { ...group, totalQuantity: 501 } });
  const result = await handler(
    event({
      httpMethod: 'POST',
      path: '/inventory',
      resource: '/inventory',
      body: JSON.stringify({
        name: 'Rice',
        category: 'Grains',
        expirationDate: '2028-01-01',
        locationId: 'pantry',
        locationDetails: 'Shelf 2A',
        quantity: 1,
        unit: 'g',
        pictureUrl: 'https://example.com/rice.jpg',
      }),
    }),
  );
  expect(result.statusCode).toBe(201);
  const put = mockSend.mock.calls.find(([command]) => command.kind === 'put')![0];
  expect(put.Item).toMatchObject({
    locationDetails: 'Shelf 2A',
    pictureUrl: 'https://example.com/rice.jpg',
  });
  expect(JSON.parse(result.body).groups[0]).toMatchObject({ threshold: 1, thresholdUnit: 'kg' });
});

it('persists threshold units and computes converted low-stock status', async () => {
  mockSend
    .mockResolvedValueOnce({ Item: { unit: 'g', totalQuantity: 1000, isLowStock: false } })
    .mockResolvedValueOnce({ Attributes: { threshold: 1, thresholdUnit: 'kg', isLowStock: true } });
  const result = await handler(
    event({
      httpMethod: 'PUT',
      resource: '/inventory/groups/{groupId}',
      pathParameters: { groupId: 'rice' },
      body: JSON.stringify({ threshold: 1, thresholdUnit: 'kg' }),
    }),
  );
  expect(result.statusCode).toBe(200);
  expect(mockSend.mock.calls[1][0].ExpressionAttributeValues).toMatchObject({
    ':threshold': 1,
    ':thresholdUnit': 'kg',
    ':low': true,
  });
  expect(JSON.parse(result.body).lowStockTransition).toBe(true);
});

it('rejects incompatible threshold units without writing', async () => {
  mockSend.mockResolvedValueOnce({ Item: { unit: 'g', totalQuantity: 1000 } });
  const result = await handler(
    event({
      httpMethod: 'PUT',
      resource: '/inventory/groups/{groupId}',
      pathParameters: { groupId: 'rice' },
      body: JSON.stringify({ threshold: 1, thresholdUnit: 'bottle' }),
    }),
  );
  expect(result.statusCode).toBe(400);
  expect(mockSend).toHaveBeenCalledTimes(1);
});

it.each(['POST', 'PUT'])('rejects non-text Location Details on %s', async (httpMethod) => {
  const result = await handler(
    event({
      httpMethod,
      resource: httpMethod === 'POST' ? '/inventory' : '/inventory/{itemId}',
      pathParameters: httpMethod === 'PUT' ? { itemId: 'rice' } : null,
      body: JSON.stringify({ locationDetails: 123 }),
    }),
  );
  expect(result.statusCode).toBe(400);
  expect(mockSend).not.toHaveBeenCalled();
});
