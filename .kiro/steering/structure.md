...
- Handler-adjacent pure functions may be extracted into a sibling module next to the handler so they can be unit- and property-tested in isolation without AWS SDK calls; the handler stays in `<feature>.ts` and imports them
