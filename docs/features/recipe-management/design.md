...
Shared infrastructure — DynamoDB table schema, auth flow, API Gateway setup, S3, sync, IndexedDB schema — is defined in the main spec at `docs/features/pantry-tracking-app/design.md` and `data-model.md`. This document covers only recipe-specific implementation detail.
