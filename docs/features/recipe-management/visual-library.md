...
Recipes Library shows compact rectangular cookbook covers together with the All recipes
list. My cookbooks shows only the cookbook shelf, several per desktop row and two per
...
book; moving or cancelling the pointer cancels the hold. Cookbooks can be moved up or
down in a device-local shelf order.
...
preserves the selected collection, filters and view. New cookbook sits beside New Recipe.
