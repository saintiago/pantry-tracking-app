...
- **Form_Field**: Any input or select element in the AddItemPage (name, category, expirationDate, locationId, quantity, unit, barcode, brand, whereToBuy, onlineStoreLink)
...
2. WHEN matching items are found, THE AddItemPage SHALL display an Autocomplete_Dropdown below the barcode field showing all matching items
...
5. WHEN the barcode field contains fewer than 3 characters, THE AddItemPage SHALL hide the Autocomplete_Dropdown
...
2. WHEN matching items are found, THE AddItemPage SHALL display an Autocomplete_Dropdown below the name field showing all matching items
3. THE Autocomplete_Dropdown SHALL display a maximum of 10 matching items
...
7. WHEN the name field contains fewer than 3 characters, THE AddItemPage SHALL hide the Autocomplete_Dropdown
8. WHEN no matching items are found, THE AddItemPage SHALL not display the Autocomplete_Dropdown
...
4. WHEN the user presses the Down Arrow key while a field with an active dropdown is focused, THE AddItemPage SHALL move focus to the first item in the Autocomplete_Dropdown
5. WHEN the user presses the Up Arrow key while navigating the Autocomplete_Dropdown, THE AddItemPage SHALL move focus to the previous item
6. WHEN the user presses the Down Arrow key while navigating the Autocomplete_Dropdown, THE AddItemPage SHALL move focus to the next item
7. WHEN the user presses Tab while navigating the Autocomplete_Dropdown, THE AddItemPage SHALL move focus to the next item (wrapping to first item at the end)
8. WHEN the user presses Shift+Tab while navigating the Autocomplete_Dropdown, THE AddItemPage SHALL move focus to the previous item (wrapping to last item at the beginning)
...
10. WHEN the user presses Escape while the Autocomplete_Dropdown is visible, THE AddItemPage SHALL hide the dropdown and return focus to the input field
11. WHEN the user clicks outside the input field and Autocomplete_Dropdown, THE AddItemPage SHALL hide the dropdown
12. WHEN an item is selected from the Autocomplete_Dropdown, THE AddItemPage SHALL hide the dropdown
...
2. WHEN matching categories are found, THE AddItemPage SHALL display an Autocomplete_Dropdown below the category field showing all matching values
...
2. WHEN matching brands are found, THE AddItemPage SHALL display an Autocomplete_Dropdown below the brand field showing all matching values
...
2. WHEN matching values are found, THE AddItemPage SHALL display an Autocomplete_Dropdown below the whereToBuy field showing all matching values
...
2. WHEN matching values are found, THE AddItemPage SHALL display an Autocomplete_Dropdown below the onlineStoreLink field showing all matching values
3. THE Autocomplete_Dropdown SHALL display a maximum of 10 matching values
...
1. WHEN a Form_Field is populated by the Autofill_System, THE AddItemPage SHALL apply a distinct visual style to indicate the field is prefilled
2. THE AddItemPage SHALL use a different background color for Prefilled_Fields compared to empty or user-entered fields
3. THE AddItemPage SHALL ensure prefilled styling meets WCAG 2.1 Level AA contrast requirements
4. THE AddItemPage SHALL apply prefilled styling to all field types (text inputs, selects, number inputs)
...
1. WHEN a user modifies a Prefilled_Field, THE AddItemPage SHALL change the field styling from prefilled to user-edited
2. THE AddItemPage SHALL detect modifications through typing, pasting, selecting dropdown options, and deleting content
3. WHEN a user clears a Prefilled_Field to empty, THE AddItemPage SHALL remove the prefilled styling
4. THE AddItemPage SHALL track edit state independently for each Form_Field
5. WHEN the modal is closed and reopened, THE AddItemPage SHALL reset all fields to their initial empty state
...
1. WHEN a lookup is in progress, THE AddItemPage SHALL display a loading indicator near the triggering field
2. WHEN a lookup fails due to network error, THE AddItemPage SHALL display an error message to the user
3. WHEN a lookup completes with no results, THE AddItemPage SHALL not display an error message
4. THE AddItemPage SHALL allow users to continue editing fields while a lookup is in progress
...
1. WHEN the form is submitted, THE AddItemPage SHALL include all Prefilled_Fields in the submission data
2. WHEN the form is submitted, THE AddItemPage SHALL include all User_Edited_Fields in the submission data
3. THE AddItemPage SHALL validate all fields according to existing validation rules regardless of whether they are prefilled or user-entered
4. WHEN required fields are not populated after autofill, THE AddItemPage SHALL display validation errors on submission
...
1. WHEN fields are autofilled, THE AddItemPage SHALL announce the autofill action to screen readers
2. THE AddItemPage SHALL maintain proper focus management during autofill operations
3. THE AddItemPage SHALL ensure all autofill-related UI elements are keyboard accessible
4. WHEN a loading indicator is displayed, THE AddItemPage SHALL provide appropriate ARIA attributes for screen readers
