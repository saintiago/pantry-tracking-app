...
This design covers two cooperating parts, both defined against the shared contracts in `docs/architecture/data-model.md`:
...
This feature introduces **no new data models**. It uses the existing `MealPlan` entity, the `CreateMealPlanRequest` interface, and the `/meal-plans` routes from `docs/architecture/data-model.md`. Summarized here for traceability only:
