...
Shared entities (`MealPlan`, `Recipe`), the DynamoDB single-table layout, and the `/meal-plans` API contract are defined in `docs/architecture/data-model.md` and are referenced here rather than redefined. Shared infrastructure terms (Pantry_App, Beautiful_User) follow the conventions used across existing specs.
