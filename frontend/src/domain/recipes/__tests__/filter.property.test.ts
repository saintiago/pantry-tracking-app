...
import { computeTotalTime } from '@pantry/domain';
      ...
      fc.property(
        ...
        (recipes, inventoryItems, filters) => {
          ...
          if (filters.maxPrepTime !== undefined) {
            perFilterResults.push(
              filterRecipes(recipes, { ...EMPTY_FILTERS, maxPrepTime: filters.maxPrepTime }, idx),
            );
          }
          if (filters.maxCookTime !== undefined) {
            perFilterResults.push(
              filterRecipes(recipes, { ...EMPTY_FILTERS, maxCookTime: filters.maxCookTime }, idx),
            );
          }
          if (filters.maxTotalTime !== undefined) {
            perFilterResults.push(
              filterRecipes(recipes, { ...EMPTY_FILTERS, maxTotalTime: filters.maxTotalTime }, idx),
            );
          }
          if (filters.onlyAllAvailable) {
            perFilterResults.push(
              filterRecipes(recipes, { ...EMPTY_FILTERS, onlyAllAvailable: true }, idx),
            );
          }
